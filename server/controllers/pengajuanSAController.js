...
const createPengajuanSA = async (req, res) => {
  try {
    ...
    if (!parsedMataKuliahIds || !Array.isArray(parsedMataKuliahIds) || parsedMataKuliahIds.length === 0) {
      return res.status(400).json({ error: 'Mata kuliah harus dipilih minimal 1' });
    }

    if (parsedMataKuliahIds.length > 3) {
      return res.status(400).json({ error: 'Maksimal 3 mata kuliah per pengajuan' });
    }
