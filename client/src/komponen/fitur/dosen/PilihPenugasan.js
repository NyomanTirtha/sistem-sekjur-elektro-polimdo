...
import { teachingAssignmentsAPI } from '../../../utilitas/api/api';
import { showSuccessAlert, showErrorAlert, showWarningAlert } from '../../../utilitas/notifikasi/alertUtils';
...
export default function TeachingPicker() {
  ...
  const onAssign = async (mataKuliahId) => {
    ...
    try {
      ...
      await loadData();
      showSuccessAlert('Pengajuan berhasil dikirim! Menunggu persetujuan Kaprodi.');
    } catch (e) {
      if (e.message?.includes('409') || /sudah diambil/i.test(e.message)) {
        const errorMsg = 'Mata kuliah sudah diambil dosen lain atau sudah ada pengajuan.';
        setError(errorMsg);
        showWarningAlert(errorMsg);
      } else {
        const errorMsg = e.message || 'Gagal mengambil mata kuliah';
        setError(errorMsg);
