...
import { Eye, EyeOff, User, Lock, AlertCircle, Shield, CheckCircle2 } from 'lucide-react';
import logoLogin from '../assets/images/polimdo-logo.png';

// ✨ TAMBAHKAN CUSTOM HOOK UNTUK TITLE
const useDocumentTitle = (title) => {
  useEffect(() => {
    // Set title dengan prefix aplikasi
    document.title = title ? `${title} - Sistem Jurusan Elektro` : 'Sistem Jurusan Elektro';
    
    // Cleanup function untuk reset title saat component unmount
    return () => {
      document.title = 'Sistem Jurusan Elektro';
    };
  }, [title]);
};

const LoginPage = ({ onLoginSuccess }) => {
  // ✨ GUNAKAN CUSTOM HOOK UNTUK SET TITLE
    ...
    try {
      ...
      if (result.success) {
        ...
        const userData = {
          ...
          ...(result.data.user.prodi && { prodi: result.data.user.prodi }),
