...
const UsersList = ({ authToken, currentUser }) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        alert(`❌ Error: ${errorData.error || 'Gagal mengambil data users'}`);
      }
    } catch (error) {
      console.error('Error fetching users:', error);
      alert('❌ Terjadi kesalahan saat mengambil data users');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(`✅ Akun berhasil ${isEditing ? 'diupdate' : 'dibuat'}!`);
        fetchUsers();
        resetForm();
      } else {
        const errorData = await response.json();
  ...
  const handleDelete = async (user) => {
    ...
    // Check current user - use username comparison as fallback
    ...
    const isConfirmed = window.confirm(
      `⚠️ PERINGATAN! Apakah Anda yakin ingin menghapus akun "${user.nama}" (${user.username})?\n\nAksi ini tidak dapat dibatalkan dan akan menghapus semua data terkait.`
    );
    ...
    const doubleConfirm = window.confirm(
      `Konfirmasi sekali lagi: Hapus akun "${user.nama}"?\n\nKlik OK jika Anda benar-benar yakin.`
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        alert(`❌ Error: ${errorData.error || 'Gagal menghapus akun'}`);
      }
    } catch (error) {
