...
export default function MahasiswaList({ authToken, currentUser }) {
  ...
  const handleDelete = async (nim) => {
    ...
    if (window.confirm('Apakah Anda yakin ingin menghapus data mahasiswa ini?')) {
      try {
        ...
        const response = await fetch(`${API_BASE}/mahasiswa/${nim}`, {
          method: 'DELETE',
          headers: getAuthHeaders()
        });
        ...
        if (response.ok) {
          ...
          const newFilteredCount = filteredMahasiswa.length - 1;
          ...
          if (response.status === 401) {
            ...
            alert(errorData.message || 'Terjadi kesalahan saat menghapus data');
          }
        }
      } catch (error) {
  ...
  const filteredMahasiswa = mahasiswa.filter(mhs => {
    ...
    const matchesProdi = filterProdi === '' || prodiData?.nama === filterProdi;
                  ...
                  {currentMahasiswa.map((mhs, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4">{prodiData?.nama || '-'}</td>
